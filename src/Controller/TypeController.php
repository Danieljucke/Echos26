...
#[Route('/dashboard/type'), IsGranted('IS_AUTHENTICATED_FULLY')]
class TypeController extends AbstractController
{
    #[Route('/liste', name: 'app_type_index', methods: ['GET'])]
    ...
    public function new(Request $request, TypeRepository $typeRepository): Response
    {
        ...
        $form->remove('createdAt')->remove('modifiedAt');
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $type->setModifiedAt(new \DateTimeImmutable('now'))
